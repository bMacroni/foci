...
describe('Google Authentication Integration', () => {
  ...
  describe('LoginScreen Google Sign-In', () => {
    ...
    it('should handle account linking required scenario', async () => {
      const mockGoogleUser = {
        idToken: 'mock-google-id-token',
        serverAuthCode: 'mock-auth-code',
        user: {
      ...
      // Mock backend response indicating linking required
      (global.fetch as jest.Mock).mockResolvedValueOnce({
        ok: false,
        status: 409,
        json: async () => ({ status: 'linking_required' }),
      });
      ...
      // Should show account linking modal
      await waitFor(() => {
  ...
  describe('Account Linking Flow', () => {
    it('should handle successful account linking', async () => {
      ...
      // Mock linking required response
      (global.fetch as jest.Mock)
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => mockLinkingResponse,
        });
      ...
      // Wait for linking modal to appear
      await waitFor(() => {
        expect(screen.getByText('Account Already Exists')).toBeTruthy();
      });
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(
          'http://localhost:5000/api/auth/google/link-account',
          expect.objectContaining({
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
      ...
      await waitFor(() => {
        expect(authService.setSession).toHaveBeenCalledWith(
          mockLinkingResponse.token,
          mockLinkingResponse.user
        );
      });
    ...
    it('should handle incorrect password during linking', async () => {
      const mockGoogleUser = {
        idToken: 'mock-google-id-token',
        serverAuthCode: 'mock-auth-code',
        user: {
      ...
      (global.fetch as jest.Mock)
        ...
        // Mock incorrect password response
        .mockResolvedValueOnce({
          ok: false,
          status: 401,
    ...
    it('should handle linking cancellation', async () => {
      const mockGoogleUser = {
        idToken: 'mock-google-id-token',
        serverAuthCode: 'mock-auth-code',
        user: {
